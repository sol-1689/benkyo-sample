...
デメリットとしては、画面のリフレッシュやURLをコピーして別タブで開かれたような場合、javascriptの変数などもクリアされてしまいます。  
そうならないように実装上工夫する必要があります。  
historyAPIというページ遷移無しURLを変更するjavascriptの関数を利用し、ページ書き換え時にパラメータ（開いているメールのidなど）をURLに追加する事が多いです。  
