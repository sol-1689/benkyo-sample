...
html以外にも画面描画の為にブラウザが使用するファイルがあります。  
cssとjavascriptと呼ばれるファイルです。  

- css  
主に画面のデザイン情報を記載します。  
デザイン情報とは、幅や高さ、色などです。  
そのような情報は、htmlタグのstyleという属性で直接指定する事も出来ます。  

例  


この例のような、backgroundcolorは背景色を表す、といった定義はW3Cという団体によって[定義](https://www.w3.org/Style/CSS/specs.en.html)されています。  

ただ、このように直接記述する方法だとタグが増えた際に管理が大変になります。
その為、styleで記載する内容をhtmlとは別のファイルにまとめておき、htmlに読み込ませることが出来ます。  
そのstyleの内容をまとめて記載するファイルがcssとなります。  
